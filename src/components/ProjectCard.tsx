...
export function ProjectCard({ project }: ProjectCardProps) {
  if (project.placeholder) {
    return (
      <div className="block border border-dashed border-zinc-300 dark:border-white/10 rounded-2xl p-6">
        <p className="text-sm text-zinc-600 dark:text-zinc-600">In progress</p>
      </div>
    );
  }
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="group flex flex-col sm:flex-row border border-zinc-300/80 dark:border-white/10 rounded-2xl overflow-hidden bg-white/45 dark:bg-transparent hover:bg-white/80 dark:hover:bg-white/5 transition-colors"
    >
      <div className="flex flex-col justify-between p-4 sm:p-6 sm:w-1/2">
        <div>
          ...
          <p className="mt-2 text-sm text-zinc-700 dark:text-zinc-400 leading-relaxed">
            {project.description}
          </p>
        </div>
